...
const { serveStatic, requestUrl } = require('./util');
...
function createAppHandler({ stile, admin, publicDir, templatesDir }) {
  return function appHandler(req, res) {
    ...
    if (p === '/health') {
      ...
      let storeOk = true;
      try {
        stile.store.events.counterToday();
      } catch (err) {
        storeOk = false;
        // Surface the cause server-side; the response body stays minimal.
        console.warn(`[stile] /health store check failed: ${err && err.message || err}`);
      }
      // A custom store is an object, not a 'memory'/'file' shorthand string —
      // don't mislabel it as in-memory.
      const cfgStore = stile.options.store;
      const storeType = typeof cfgStore === 'string'
        ? (cfgStore.startsWith('file') ? 'file' : 'memory')
        : cfgStore ? 'custom' : 'memory';
