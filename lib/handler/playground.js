...
\`\`\``;
    ...
    // Use the atomic consume() if available; fall back to has()+add() for
    // older custom stores.
    let ok;
    if (typeof stile.store.nonces.consume === 'function') {
      ok = stile.store.nonces.consume(challenge.nonce, challenge.exp);
    } else if (stile.store.nonces.has(challenge.nonce)) {
      ok = false;
    } else {
      stile.store.nonces.add(challenge.nonce, challenge.exp);
      ok = true;
    }
    const verifyResp = ok ? {
...
// OpenAI-compatible chat-completions providers — identical request/response
// shape, differing only by endpoint and default model.
const OPENAI_COMPATIBLE = {
  openai:     { url: 'https://api.openai.com/v1/chat/completions',           defaultModel: 'gpt-4o-mini' },
  openrouter: { url: 'https://openrouter.ai/api/v1/chat/completions',        defaultModel: 'meta-llama/llama-3.1-8b-instruct' },
  cerebras:   { url: 'https://api.cerebras.ai/v1/chat/completions',          defaultModel: 'llama3.1-8b' },
  groq:       { url: 'https://api.groq.com/openai/v1/chat/completions',      defaultModel: 'llama-3.1-8b-instant' },
  nvidia:     { url: 'https://integrate.api.nvidia.com/v1/chat/completions', defaultModel: 'meta/llama-3.1-8b-instruct' },
};

function streamTokens(text, send) {
