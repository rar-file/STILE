...
const SCHEMA = [
  ...
  // ---- ip hash salt -------------------------------------------------------
  {
    ...
    resolve(raw, ctx, out) {
      if (!raw) {
        if (ctx.isProd) {
          out.fail(
            'STILE_IP_SALT is unset. IP hashes use a public default salt and will correlate ' +
            'across deployments, enabling cross-instance event attribution. ' +
            'Generate one: openssl rand -hex 32',
            'public default salt — set STILE_IP_SALT');
        } else {
          // Ephemeral salt keeps dev hashes off the public default without
          // requiring operators to configure anything locally.
          out.value = crypto.randomBytes(16).toString('hex');
          out.warn(
            'STILE_IP_SALT unset; using an ephemeral random salt. ' +
            'IP hashes will not correlate across restarts. Set STILE_IP_SALT for stable attribution.',
            'ephemeral random (dev)');
        }
        return;
      }
  ...
  // ---- store --------------------------------------------------------------
  {
    ...
    env: 'STILE_STORE',
    resolve(raw, ctx, out) {
      if (!raw) raw = ctx.isProd ? 'memory' : 'memory';
