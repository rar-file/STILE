...
function createMemoryStore({
  ...
  // ---- rate-limit counters (opt-in, in-memory only)
  const rateLimitCounters = new Map(); // key -> { count, windowStart }
  ...
  return {
    nonces: {
      ...
      // Atomic check-and-add. Returns true if the nonce was new and has been
      // recorded; false if it was already present (replay attempt).
    ...
    rateLimits: {
      // Fixed-window counter. Returns the request count for this key in the
      // current window — caller compares against maxAttempts.
      hit(key, windowMs) {
        const now = Date.now();
