...
function createFileStore({
  ...
  // ---- rate limits (#6) ----
  // key → { count, expiresAt(ms) }. Persisted so a process restart cannot
  // erase an attacker's history within the current window.
  const rateLimitWindows = new Map();
  for (const [k, w] of Object.entries(raw.rateLimits || {})) {
    if (w && typeof w.expiresAt === 'number' && w.expiresAt > Date.now()) rateLimitWindows.set(k, w);
  }
  const rateLimitCleanup = setInterval(() => {
    const now = Date.now();
    let removed = 0;
    for (const [k, w] of rateLimitWindows) if (w.expiresAt <= now) { rateLimitWindows.delete(k); removed++; }
  ...
  function snapshot() {
    return {
      ...
      rateLimits: Object.fromEntries(rateLimitWindows),
    };
  }
  ...
  return {
    nonces: {
      ...
      // Atomic check-and-add. The Node event loop guarantees that this body
      // runs to completion without interleaving with another consume() in the
      // same process, so the Map read and write are effectively a critical
      // section. Two processes pointing at the same file remain unsafe — this
      // store is single-writer (see top-of-file note); use a shared store for
      // multi-process deployments.
      consume(n, expSec) {
        if (usedNonces.has(n)) return false;
        usedNonces.set(n, expSec * 1000);
        markDirty();
        return true;
      },
    },
    ...
    rateLimits: {
      ...
      reset(key) { if (rateLimitWindows.delete(key)) markDirty(); },
    },
    adopters: {
      upsert(domain, info = {}) {
