...
const DEFAULTS = {
  ...
  rateLimit: null,              // { windowMs, maxAttempts } — opt-in; requires store.rateLimits
};
...
function createStile(userOpts = {}) {
  ...
  if (opts.rateLimit && !store.rateLimits) {
    console.warn('[stile] rateLimit option is set but the configured store does not implement store.rateLimits — rate limiting is disabled.');
  }
...
${phrasingHtml}
  ...
  async function handleVerify(req, res, urlObj) {
    // Rate limiting — checked before body parsing so flooded IPs are dropped cheaply.
    if (opts.rateLimit && store.rateLimits) {
      const key = ipHash(remoteIp(req), opts.ipHashSecret);
      if (key) {
        const count = store.rateLimits.hit(key, opts.rateLimit.windowMs);
        if (count > opts.rateLimit.maxAttempts) {
          const retryAfter = Math.ceil(opts.rateLimit.windowMs / 1000);
          res.statusCode = 429;
          res.setHeader('Content-Type', 'application/json');
          res.setHeader('Retry-After', String(retryAfter));
          res.end(JSON.stringify({ error: 'rate_limit_exceeded', retry_after: retryAfter }));
          return true;
        }
      }
    }
