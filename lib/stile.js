...
const { b64url, hmac, safeEq, htmlEscape, requestUrl, readBody } = require('./util');
const { version: STILE_VERSION } = require('../package.json');
...
const DEFAULTS = {
  ...
  rateLimit: null,              // null = disabled; { windowMs, maxAttempts } enables store-backed limiting on the verify endpoint
  trustProxy: true,             // true: derive client IP from X-Forwarded-For; false: use the socket peer (resists XFF spoofing)
};
...
function parseCookies(header) {
  ...
  for (const part of header.split(';')) {
    ...
    // A malformed percent-encoding (e.g. "stile=%") makes decodeURIComponent
    // throw. Without a guard that URIError propagates out of the ungated gate()
    // and crashes the process — a one-request DoS. Fall back to the raw value.
    if (k) {
...
function ipHash(ip, secret) {
  if (!ip) return null;
  // Caller is required to pass a real salt. createStile() now synthesizes one
  // when ipHashSecret is unset, so this branch should never fire in practice;
  // we keep a defensive bail-out so the function never silently emits a hash
  // computed under a publicly-known key.
...
function createStile(userOpts = {}) {
  ...
  // Never silently fall back to a published default for the IP-hash salt — if
  // the caller didn't supply one (or the config layer asked us to generate
  // one in dev), synthesize a fresh random salt now. Production callers must
  // come through config.load() which fails closed when STILE_IP_SALT is unset.
  if (!opts.ipHashSecret) {
    opts.ipHashSecret = crypto.randomBytes(32).toString('hex');
  }
  if (opts.tier === 'strong') {
  ...
  // Client IP used for rate-limit keys and event hashing. When trustProxy is
  // false we ignore X-Forwarded-For and use the socket peer, so a client can't
  // mint a fresh rate-limit bucket per request by rotating the header. IP
  // allow/deny rules still read XFF (they are operator-configured and assume a
  // trusted edge); see docs/THREAT_MODEL.md T10.
  const clientIp = (req) => opts.trustProxy === false
    ? (req.socket && req.socket.remoteAddress) || null
    : remoteIp(req);

  // Resolve rate-limit policy. Disabled by default. When configured against a
  // store that doesn't implement rateLimits, we warn once at construction
  // time and act as if it were disabled — failing closed on a typo here would
  // surprise operators upgrading custom stores.
  let rateLimit = null;
  if (opts.rateLimit) {
    const { windowMs, maxAttempts } = opts.rateLimit;
    if (!Number.isFinite(windowMs) || windowMs <= 0 || !Number.isFinite(maxAttempts) || maxAttempts <= 0) {
      throw new Error('[stile] rateLimit requires { windowMs > 0, maxAttempts > 0 }');
    }
    if (!store.rateLimits || typeof store.rateLimits.hit !== 'function') {
      console.warn('[stile] rateLimit option set but the configured store has no rateLimits namespace; rate limiting is disabled. Implement store.rateLimits.hit(key, windowMs) to enable it.');
    } else {
      rateLimit = { windowMs, maxAttempts };
    }
  }
...
${phrasingHtml}
  ...
  async function handleVerify(req, res, urlObj) {
    // Rate limit before any signature work: an attacker shouldn't be able to
    // burn CPU on HMAC verification just by spamming this endpoint. Keyed by
    // the IP hash so it composes with whatever proxy is in front of us, and
    // so an operator can correlate a 429 back to an event ip_hash if needed.
    if (rateLimit) {
      const ip = clientIp(req);
      const key = `verify:${ipHash(ip, opts.ipHashSecret) || 'anon'}`;
      const { count, expiresAt } = store.rateLimits.hit(key, rateLimit.windowMs);
      if (count > rateLimit.maxAttempts) {
        ...
        res.end(JSON.stringify({
          ...
          hint: `Too many verify attempts. Try again in ${retryAfter}s.`,
        }));
        return true;
      }
    }
    ...
    // Single-use nonce (#18). Prefer the atomic consume() if the store
    // implements it — otherwise fall back to has()+add() so custom stores
    // written against the older contract keep working. The two-call fallback
    // is single-process-safe but races across processes; see docs/API.md §4.
    let nonceAccepted;
    if (typeof store.nonces.consume === 'function') {
      nonceAccepted = store.nonces.consume(claim.nonce, claim.exp);
    } else {
      if (store.nonces.has(claim.nonce)) {
        nonceAccepted = false;
      } else {
        store.nonces.add(claim.nonce, claim.exp);
        nonceAccepted = true;
      }
    }
    if (!nonceAccepted) {
    ...
    if (fast) {
      ...
      if (second.decision !== 'deny') {
        ...
        store.events.record({
          ...
          tier: 'fast-path',
          ip_hash: ipHash(clientIp(req), opts.ipHashSecret),
          ts: Date.now(),
        });
...
<div class="shell">
  ...
  <div class="legend">
    ...
    <span>STILE / v${STILE_VERSION}</span>
  </div>
</div>
${block}
</body></html>`;
